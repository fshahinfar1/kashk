...
#     # debug('Investigate:', inst.name, tag=MODULE_TAG)
...
#         if func.may_fail:
#             # debug(MODULE_TAG, f'function {func.name} may fail!', tag=MODULE_TAG)
...
#             # debug(MODULE_TAG, 'Code selected for after calling the failing function:', rest_of_the_code, tag=MODULE_TAG)
...
#             # debug('reach "to user space inst"', tag=MODULE_TAG)
#             _set_in_userland(more)
...
#                         # debug(MODULE_TAG, 'new boundry era', tag=MODULE_TAG)
...
#                     # debug(MODULE_TAG, 'TO USERSPACE dead', to_user_inst_ref.path_id, tag=MODULE_TAG)
#                 else:
...
# def select_user_pass_orig(inst, info, more):
#     # Initialize the pass
#     _init_userland_signal(more)
